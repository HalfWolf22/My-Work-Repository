...
I work a lot so it gets updated from time to time.
It is mainly made up of little functions that are useful for data science,
...
def degrade_columns(df, sep, filter=False, filter_threshold=0.0456, return_ftr_list=False,
    ...
    if filter == True:
        ...
        for feature in new_features:
            if ((df.groupby(feature)[feature].count()/len(df))[0] <= 0.0456)|(
            (df.groupby(feature)[feature].count()/len(df))[1] <= 0.0456):
                df = df.drop(feature, axis=1)
                if debug == True:
                    print('Dropped ', feature)
            else:
...
def look(df, pred=None):
    ...
    if pred is None:
        ...
        mtx = pd.concat([types, counts, distincts, nulls, missing_ratio, uniques, skewness], axis=1)
...
def plot_spread(df, cols):
    ...
    for column in cols:
        ...
        if df[column].nunique() < 20:
            plt.subplot(1, 2, 1)
            sns.distplot(df[column], kde_kws={'bw':1})
            plt.title('Histogram')
        else:
            plt.subplot(1, 2, 1)
