...
# Matches to scrape
with open('matches_eune.csv', newline='') as file:
    r = csv.reader(file)
...
def get_data():
    ...
    with open(csv_name, 'w', newline='', encoding='utf-8') as out_file:
        ...
        for idx, match in enumerate(matches[:10000]):  # Put indices to scrape here
            if idx != 0:
                if idx % 100 == 0:
                    print('\n')
            ...
            match_url = 'https://eun1.api.riotgames.com/lol/match/v4/matches/' + str(match) + '?api_key=' + key
