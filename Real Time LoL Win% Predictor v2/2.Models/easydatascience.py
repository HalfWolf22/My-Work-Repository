...
Note:
    ??? Use correlation matrix and print one of 2 triangles.
    Add option to choose features
"""
...
Parameters:
    series - pandas.Series, column to plot
    color - plotting color
    stats -  bool value, if True prints the skewness, mean and std of the series

Note:
    /
"""
def plot_spread(series, color='b', stats=True):
    import numpy as np
    import pandas as pd
    import matplotlib.pyplot as plt
    ...
    if series.nunique() < 60:
        sns.distplot(series, kde_kws={'bw':1}, color=color, ax=ax[0])
    else:
        sns.distplot(series, color=color, ax=ax[0])

    ax[0].set_title('Histogram')

    sns.boxplot(series, color=color, ax=ax[1])
    ...
    if stats == True:
        ...
        print('Mean: ', round(series.mean(), 4))
